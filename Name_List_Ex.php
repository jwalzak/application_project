<?php

    class name_list extends input_data {
    //Four arrays of first names, and last names for two genders.
        
    //http://fantasywritersresource.blogspot.ca/2012/09/beautifiul-rare-and-unique-female-names.html

    //http://medieval.stormthecastle.com/medieval-names.htm

    $first_name_f = arr("Leanne", "Sonya", "Heather", "Glenda", "Abella", "Aililve", "Alura", "Calista", "Carita", "Cein" "Chrissanth", "Cleantha", "Delica", "Deva", "Elidie", "Evvie", "Eliska", "Faye", "Faylinn", "Gaea");
    $first_name_m = arr("Isaiah", "Jason", "Matt", "Qole", "Merek", "Carac", "Ulric", "Rowan", "Althalos", "Brom", "Grom", "Jarin", "Joturn", "Asher", "Frodo" );
    $last_name_f = arr();
    $last_name_m = arr();
}


?>